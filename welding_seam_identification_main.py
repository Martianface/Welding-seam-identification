...
find the difference between theretical and actual welding seam
...
f_laser_data = open("./31.1.txt", "r") # reading laser data from a txt file
f_laser_data_filtered = open("./31.1_f.txt", "w") # wrting filtered data to a txt file
# laser data format is x and z coordinates
x = []
z = []
point_set = []
...
for line in f_laser_data.readlines():
    ...
    if len(raw_data) == 4 and raw_data[1] != '-64.3914':
        # removing the data out of range as '-64.3914'
        x.append(float(raw_data[0]))
        z.append(float(raw_data[1]))
        point_set.append([x,z])
...
# the ending condition is that the number of point set is less than a threshold
point_number_threshold = 2
while (len(point_set)<=point_number_threshold):
    pass
